...
**Do this first, and do only this:** open the file at the exact path `.github/prompts/scrape.prompt.md` with your file-reading tool (the `view` / read-file tool), then follow its instructions exactly and in full. That prompt file is the authoritative specification for this workflow — follow every step.
